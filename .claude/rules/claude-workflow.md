...
>
> 本檔僅供 Claude Code 使用（其他代理請忽略）。使用者 2026-07-14 定案、2026-07-18 擴充定案（架構設計、對抗審查格式、驗收紀律）、2026-07-20 擴充定案（降級產出 registry）、**2026-07-26 修訂定案（fable 使用範圍收斂至資安領域、架構設計改 opus）**、**2026-07-28 定案（位階＝本檔上位／全域為 fallback；階梯標題改為「通用三級＋fable 領域專用」；明文終局性判準；對抗審查列為常設授權；補共享接縫清單的權威落點指標）**、**2026-07-28 更正（〈降級產出 registry〉新增過渡條文二：2026-07-26「架構設計改 opus」的前提是 Opus 5，故 Opus 4.8 產出／覆核的架構設計仍屬降級、須以 Opus 5 重推——**有界，僅限此次模型換代，不設計為通則**）**；另一專案的具體示例已改寫為本專案原生條文。
...
🔴 **過渡條文二（2026-07-28 使用者更正）——有界，不是通則**：上條「架構設計改 opus」的**前提是 Opus 5 的能力提升**。故在 Opus 5 到位前、以**更早世代 opus（Opus 4.8）產出或覆核**的架構設計，**不算已達應有階級**，仍須以 Opus 5 重新推導比較一次才可結案（概念同原先的「待 fable 重跑」，只是覆核者換成 Opus 5）；若有條目係以更早世代 opus 覆核並已標記完成，一律打回 `待覆核`。
**適用範圍僅限此一次模型換代。** 這類盤點只在 Claude 官方更新可用模型時才會發生，**不設計為通則**——本檔因此**不**定義「opus 恆指當前版本」之類的常設規則，也不建立隨版本滾動的追溯登記義務。對應的一次性盤點屬規劃工作，已排入 `handoff/`。
🔴 **日後同型狀況的處理方式（2026-07-28 使用者定案）**：當**可用 model 變更**、導致「**尚未重作推導的區塊需要規劃用什麼設定推導**」時，**一律以開 handoff 工作包規劃新做法的方式處理**——逐次評估範圍與風險，**不在本檔加自動生效的條文義務**。本次的過渡條文二＋對應工作包即此作法的先例。
