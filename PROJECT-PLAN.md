...
> 狀態：`0.6.3` 已發行；**`0.6.x` 收斂六項已於 2026-07-26 全數驗收通過**（三輪七個 run 的播測證據見〈階段 7〉與本機 `ACCEPTANCE-0.6.x.local.md`）；後續工作＝0.6.3 播測新發現的條文缺口待 decision-board 定案，仍先不進 `0.7.0`。歷版摘要見下表——結構化狀態列為 2026-07-19 定案（C2）於本檔試點，取代逐版附加長句，防版本字串遺漏未更新。
...
| `0.6.3` | 2026-07-20 | 三 BLOCKER 定案落地：correction 完整重述義務（N1）＋私有檔案的存在本身即機密〔含健檢工具私有字串掃描〕（R1）＋revision 仲裁「整份取代＋衝突備份」（R2）；源自 0.6.2 版 Run 3／Run 4 等效模擬 |
...
- [x] **防呆條文包（B1–B5＋D1）**：DATA-SCHEMA〈通用規則〉補原子寫入建議慣例（B1，消除下游讀取者 torn read）；PLAYBOOK〈初始化〉補「玩家隨時可 OOC 喊停／迴避、主持人立即淡出不追問理由」明文句（B2）；DATA-SCHEMA〈場景與 `scene_id`〉＋PLAYBOOK 每回合第 6 步補**場景切換判準**（B3——兩輪播測 `archive/` 未觸發的根因＝判準模糊）；〈摘要〉補玩家人工編輯優先句（B4 核對結果：壓縮時機已錨定場景邊界、僅此句缺）；RAG-PROTOCOL 失效判準補「嵌入模型／版本無法確認即失效」（B5；`embedding_model` 選配欄延後、屬 schema 變更另議）；發行包 README 補戰役備份／分享慣例（D1：完整備份／閱讀分享刪 `game/private/`／換手主持信任轉移三情境＋匯出前記版本提醒）。（本項已隨 PR #25 落地並經三 lens 對抗審查；**使用者 2026-07-26 驗收通過**——B3 場景切換判準另有 0.6.3 播測正面實證：同型難判例在 0.6.2 判錯、0.6.3 判對。）
- [x] **文件債（C1／C2／D3＋版本行修正）**：DESIGN 狀態列版本字串修正（原誤留「`0.5.0` 已發行」——`0.6.0` 收版時漏改版本本體的事實錯誤）；DESIGN §6 補戰役收尾指路句＋§8 補收尾驗收句（C1 最小組合，指路寫法防雙準源）；PLAN 狀態列結構化表格試點（C2，本檔）；Claude 專屬 rules 檔補 fable 不可用時架構設計 fallback 句（D3）。（本項已隨 PR #24 落地；**使用者 2026-07-26 驗收通過**。D3 該句其後於 2026-07-26 隨「fable 收斂至資安領域、架構設計改 opus」定案改寫，見 PR #35。）
- [x] **收版 `0.6.1`**（D4）：條文包＋文件債落地後收版（`template.json`／發行包 README 版本行／DESIGN 狀態列／本表同步）；補測發現缺陷視情況 `0.6.2+`。（2026-07-19 收版落地；**使用者 2026-07-26 驗收通過**。後續已續收 `0.6.2`／`0.6.3`。）
- [x] **`0.6.0` 補測輪（A1 拆兩輪）**：Run 3＝`archive/` 觸發劇本（playground 本地素材，A3；dist 第二內附場景另議）＋短團收尾全流程（完結→尾聲→續玩接新弧）；Run 4＝共窗多人 Hot Seat 雙玩家＋host-log 刻意例外情境；檢核清單比照 0.5.0 模式置 playground 本機不追蹤檔。A2 定調：回應速度／出戲感僅由使用者真人跑團主觀回報、不再嘗試自動化評測。結果由使用者驗收後回收；若 Run 3 仍未觸發 `archive/`，視為判準文件仍模糊的訊號、優先文件澄清而非加排播測。（**共執行三輪、七個 run；使用者 2026-07-26 驗收通過**。三輪誠實現況如下——

  **① 2026-07-19（0.6.1 協定）**：首次跑 Run 3／Run 4，當時回報「archive 於場景切換觸發、短團收尾＋續玩全流程走完」。**此結論其後被推翻**：0.6.2 版重跑以目錄 mtime 鑑識證明 `archive/npcs/` 是跑完後離線補正、`campaign_status: concluded` 從未被證明在磁碟上存在過、場景切換難判例判錯。原文字係假綠，2026-07-26 依此更正。

  **② 2026-07-20（0.6.2 實際發行資產）**：`0.6.2-sim`（correction 專項，含 v0.6.1 雙對照臂）＋ Run 3／Run 4 等效模擬。**L93 當時不可勾**——Run 3 三項核對點未過、Run 4 一項紅線違反（host-log 存在洩漏到玩家可見層）＋一項條文歧義（仲裁選項二）。四缺口中三項定案後收為 `0.6.3`（N1／R1／R2）。
...
- [x] **`0.6.2` journal 回饋落地（2026-07-19 定案）**：源自 worldthread-journal `CORE-FEEDBACK.local.md`（cabalist 外部玩家 core v0.6.0 實測）三項——**回饋 1**＝correction 撤銷語意攤平（`corrects` 一律指向非-correction 原事件、同一原事件最新 correction 為有效版本；DATA-SCHEMA 事件日誌節改寫＋撤銷正例）；**回饋 3**（次要，隨回饋 1）＝溯源欄位指向仍有效事件；**回饋 2**＝PLAYBOOK〈每回合〉第 5 步補「寫入即驗證可解析」＋新增雙 mjs/py 狀態健檢工具 `tools/healthcheck.*`（＋fixtures＋`scripts/test-healthcheck-contract.ps1`＋CI／verify-package 掛載）。收版四處同步至 `0.6.2`。（本項已隨 `fix/0.6.2-journal-feedback` 落地、verify-package＋雙契約測試綠、並經三 lens 對抗審查修正 correction 範例衝突與測試覆蓋缺口；**使用者 2026-07-26 驗收通過**。播測補充：回饋 1 有 0.6.1 雙對照臂實證；**回饋 3〔溯源欄位〕原為零覆蓋，已於 0.6.3 播測首次獲非平凡覆蓋並通過**；回饋 2 含負向測試。）
...
- **契約物理拆分（T2b）＝訊號驅動晉升**（2026-07-19 使用者定案「拓撲題 0 選項 A」的第二段；第一段 T1 邏輯拆分屬待定案的機制題，不在本 backlog）。四項晉升訊號，任一成立即重新評估是否把契約拆出獨立 repo：①bot 或第二個消費端在實測中出現引用摩擦 ②出現第二個範本變體，或第三方內容作者 ③協定進入凍結期（版本節奏顯著放慢） ④`extras/` 重量在 T1 緩解後仍造成痛點。前置研究＝本機 `WORLDTHREAD-CORE-SPLIT-RESEARCH.local.md`（歷史重放實證：內容叢 0 次獨立變更、T2b 71% 跨 repo 成對 PR，故現況不拆）。
