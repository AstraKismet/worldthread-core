...
1. **私有隔離**：`game/private/director/` 是導演祕密（前線、伏筆、NPC 未揭露資訊）。僅在主持需要時讀取；其內容**永不得**出現在玩家可見的敘事、摘要或任何輸出中。**該目錄下任何檔案的存在、檔名與紀錄 id 同樣是機密**——不得寫進 `events.jsonl` 的 `ruling` 欄、實體檔 `notes`、摘要或場景檔（玩家可自行開啟 `game/state/` 的任何檔案，凡寫進去的就等於說出口）。未開局前不要主動瀏覽它。混合可見度檔案（如 `game/state/logs/events.jsonl`）內也有導演祕密行，輸出前必須依 `protocol/DATA-SCHEMA.md`〈事件日誌〉的過濾義務處理。
