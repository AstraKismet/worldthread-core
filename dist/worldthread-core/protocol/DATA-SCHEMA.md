...
本節定義**跨版本讀取存檔**的行為原則：哪些相容性事實已經確立、遇到舊格式或未知內容時如何讀。**本範本不提供、也不需要任何存檔遷移程式**——存檔的跨版本相容由本節與各節條文的讀取規則保證。範本自身的版本見發行包根目錄 `template.json`（唯讀）。

**本節射程限於存檔的讀取**——`game/state/` 各檔，以及 `game/private/director/` 底下同樣依本檔規範的狀態檔（如前線檔，見〈前線〉）。把整個資料夾的**範本本體**換成新版（`protocol/`、`tools/` 等）是另一件事：那由**玩家自行決定時機、自行執行**（可跳過中間版本、直接換到最新版），步驟見發行包根目錄 `README.md`〈升級到新版〉。**你不得代玩家執行該流程**——尤其不得覆蓋或刪除 `protocol/` 內任何檔案（見發行包 `AGENTS.md`〈行為規範〉）；玩家問起時指路即可。本節說「不需要任何遷移動作」指的是**存檔本身不必改寫**，不是「換新版時什麼都不用做」。
